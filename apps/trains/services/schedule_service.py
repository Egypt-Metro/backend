...
from ..models.schedule import ActualSchedule, Schedule
...
class ScheduleService:
    ...
    def get_train_schedule(self, train_id: str, schedule_date: date) -> QuerySet[Schedule]:
        ...
        return (
            Schedule.objects.filter(train_id=train_id, day_type=day_type, is_active=True)
            .select_related("station")
            .order_by("sequence_number")
        )

    def get_station_schedule(self, station_id: int, schedule_date: date) -> QuerySet[Schedule]:
        """
        ...
        day_type = self._get_day_type(schedule_date)
        return (
            Schedule.objects.filter(station_id=station_id, day_type=day_type, is_active=True)
            .select_related("train")
            .order_by("arrival_time")
        )

    def record_actual_arrival(
        self, schedule_id: int, arrival_time: datetime, reason: Optional[str] = None
    ) -> ActualSchedule:
        """
        ...
        try:
            ...
            actual = ActualSchedule.objects.create(schedule=schedule, actual_arrival=arrival_time, reason=reason)
    ...
    def _calculate_delay(self, actual_schedule: ActualSchedule) -> None:
        ...
        """
        scheduled_time = datetime.combine(actual_schedule.actual_arrival.date(), actual_schedule.schedule.arrival_time)
