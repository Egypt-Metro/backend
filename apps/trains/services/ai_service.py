...
class AIService:
    ...
    async def process_camera_feed(self, camera_id, image_data):
        ...
        try:
            ...
            payload = {"camera_id": camera_id, "image_data": image_data, "timestamp": timezone.now().isoformat()}

            # Make request to AI service
            response = await self._make_api_request(
                endpoint="/process-crowd", method="POST", data=payload, headers=headers
            )
    ...
    async def get_crowd_prediction(self, train_car_id, timestamp):
        ...
        try:
            ...
            response = await self._make_api_request(
                ...
                method="GET",
                params={"train_car_id": train_car_id, "timestamp": timestamp.isoformat()},
            )
