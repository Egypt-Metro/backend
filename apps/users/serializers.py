...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ['email', 'username', 'password', 'confirm_password', 'national_id', 'phone_number']
    ...
    def validate(self, data):
        ...
        if data.get('phone_number') and len(data['phone_number']) != 11:
            raise serializers.ValidationError({"phone_number": "Phone number must be 11 digits."})
        return data
...
class UserSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = [
            ...
            'payment_method', 'balance', 'created_at', 'updated_at'
        ]
        # Ensure sensitive fields (like password) are not exposed
        read_only_fields = ['password']  # Password should not be returned in the response
